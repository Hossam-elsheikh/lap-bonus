-- Fix tier upgrade to trigger when points reach (>=) max_points instead of exceeding (>)
-- This ensures users are promoted to the next tier at the appropriate threshold

DROP FUNCTION IF EXISTS public.process_test_result(uuid, uuid, text, numeric, text, timestamp with time zone);
...
BEGIN
  ...
  -- 5. Check for tier upgrade (>= instead of > to trigger at threshold)
