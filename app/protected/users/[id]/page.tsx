...
export default async function UserProfilePage({
  ...
  const calculateConversionRate = (cr: number) => {
    // Assuming CR is stored as a number (e.g., 5 for 5%)
    // If it requires calculation, adjust here.
    // Based on previous turn, "divided by pcr" implies pcr might be an integer like 10.
    return cr ? `${cr}%` : "N/A";
  };

  const tierTitle = user.tier?.title || "No Tier";
      ...
      <div className="space-y-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {tests?.length || 0} tests completed
            </CardDescription>
          </CardHeader>
          <CardContent>
